...
SocketSystem::SocketSystem()
{
  ...
  if (iResult != 0) {
	  Display::displayString("WSAStartup failed with error = " + toString(iResult));
  }
}
...
SocketSystem::~SocketSystem()
{
  ...
  Display::displayString("Socket System cleaning up");
}
...
{
	//Display::displayString("SocketConnecter instance destroyed");
}
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
  ...
  std::string sPort = toString(uport); // Resolve the server address and port
  const char* pTemp = ip.c_str();
  iResult = getaddrinfo(pTemp, sPort.c_str(), &hints, &result);  // was DEFAULT_PORT
  if (iResult != 0) {
	  Display::displayString("\n  -- getaddrinfo failed with error: " + toString(iResult));
    return false;
  } // Attempt to connect to an address until one succeeds
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
	char ipstr[INET6_ADDRSTRLEN];
    void *addr;
    char *ipver; // get pointer to address - different fields in IPv4 and IPv6:
    ...
    inet_ntop(ptr->ai_family, addr, ipstr, sizeof ipstr);// printf("\n  %s: %s", ipver, ipstr);// Create a SOCKET for connecting to server
    socket_ = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
    if (socket_ == INVALID_SOCKET) {
      int error = WSAGetLastError();
	  Display::displayString("\n  -- socket failed with error: " + toString<int>(error));
      return false;
    }
    ...
    if (iResult == SOCKET_ERROR) {
      ...
      int error = WSAGetLastError();
	  Display::displayString("WSAGetLastError returned " + toString(error));
      continue;
    }break;
  }freeaddrinfo(result);
  if (socket_ == INVALID_SOCKET) {
...
{
	Display::displayString("SocketListener instance destroyed");
}
...
bool SocketListener::bind()
{
  ...
  for (auto pResult = result; pResult != NULL; pResult = pResult->ai_next)
  {
    ...
    if (socket_ == INVALID_SOCKET) {
      int error = WSAGetLastError();
	  Display::displayString("\n  -- socket failed with error: " + toString(error));
      continue;
    }
	Display::displayString("server created ListenSocket");
    ...
    if (iResult == SOCKET_ERROR) {
      int error = WSAGetLastError();
	  Display::displayString("\n  -- bind failed with error: " + toString(error));
      socket_ = INVALID_SOCKET;
      continue;
    }
    else
...
bool SocketListener::listen()
{
  ...
  if (iResult == SOCKET_ERROR) {
    int error = WSAGetLastError();
	Display::displayString("\n  -- listen failed with error: " + toString(error));
    socket_ = INVALID_SOCKET;
    return false;
  }
  Display::displayString("server TCP listening socket setup complete");
  return true;
}
...
Socket SocketListener::accept()
{
  ...
  if (!clientSocket.validState()) {
    acceptFailed_ = true;
    int error = WSAGetLastError();
	Display::displayString("\n  -- server accept failed with error: " + toString(error));
    return clientSocket;
  }
  return clientSocket;
}
...
void ClientHandler::operator()(Socket& socket_)
{
  while (true)
  {
    ...
    if (command.size() == 0)
      break;
	Display::displayString("command: " + command);
    
    if (command == "TEST_STRING_HANDLING")
    {
      if (testStringHandling(socket_))
		  Display::displayString("----String Handling test passed");
      else
		  Display::displayString("----String Handling test failed");
      continue; // go back and get another command
    }
    if (command == "TEST_BUFFER_HANDLING")
    {
      if (testBufferHandling(socket_))
		  Display::displayString("----Buffer Handling test passed");
      else
		  Display::displayString("----Buffer Handling test failed");
  ...
  Display::displayString("ClientHandler socket connection closing");
  socket_.shutDown();
  socket_.close();
  ...
  while (true)
  {
    ...
    if (str.size() > 0)
    {
      ...
      //Verbose::show("bytes recvd at server: " + toString(str.size() + 1));
		...
		Display::displayString("server recvd: " + str);
...
bool ClientHandler::testBufferHandling(Socket& socket_)
{
  ...
  while (true)
  {
	...
	Display::displayString("server recvd: " + temp);
...
void clientTestStringHandling(Socket& si)
{
  ...
  for (size_t i = 0; i < 5; ++i)
  {
    ...
    si.sendString(text);
	Display::displayString("client sending string: " + text);
  }
  ...
  if (si.waitForData(100, 10))
  {
    while (true)
    {
      ...
      if (str.size() == 0 || str == "TEST_END")
        break;
	  Display::displayString("client recvd: " + str);
    }
  }
  else
  {
	  Display::displayString("client wait for server timed-out");
  }
}
...
void clientTestBufferHandling(Socket& si)
{
  ...
  for (size_t i = 0; i < 5; ++i)
  {
	...
	Display::displayString("client sending buffer: " + std::string(buffer));
  }
